...
def propagate_slice_1mirror(fld_slice, npadx,     # fld slice in spectral space, (Ek, x, y)
                             ...
                             Ldrift,          # Add a drift
                             misalignQ, M,    # Angular misalignment
                             roughnessQ, C,   # surface roughness
                             verboseQ): 
...
def propagate_1mirror_mpi(zsep, ncar, dgrid, nslice, xlamds=1.261043e-10,           # dfl params
                             ...
                             misalignQ = 0, M = 0,        # misalign params
                             roughnessQ = 0, C = 0, # surface roughness alignment
    ...
    for k in range(fld_block.shape[0]):   
        ...
        fld_slice, fld_slice_transmit = propagate_slice_1mirror(fld_slice = fld_slice, npadx = npadx,     
                             ...
                             misalignQ = misalignQ, M = M,        # misalignment parameter
                             roughnessQ = roughnessQ, C = C,               # roughness parameter
                             verboseQ = False)
...
propagate_1mirror_mpi(**params_dic)
